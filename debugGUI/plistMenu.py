...
def disableClientSettings():			
	...
	if gv.root:
		gv.root.title(f'{con.DEBUGGER_TITLE}: disconnected')
...
_SIGNATURECMD = f'console.script["{con.SESSION_SIGNATURE}"]'
...
def _sessionStarted(*args):
	...
	if len(startTime):					# session started
		...
		msg += f'pendingMessages: {len(gv.pendingMessages)}'
		print(msg)
...
def restoreMsgTraffic():
	...
	cmd = f'console.script["{con.SESSION_SIGNATURE}"]'
