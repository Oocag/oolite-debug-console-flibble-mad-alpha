...
class Section:
	...
	def __getitem__(self, key):
		if key not in self.options:
			raise KeyError(f'key {key!r} not in options')
	...
	def __reversed__(self):
		yield from reversed(self.options.keys())
...
class Option:
	...
	def __repr__(self):
		...
		rpt += f'    value: {self.value!r}\n'
		for text in self.comments:
			rpt += f'  comment: {text!r}\n'
		return rpt

# noinspection PyAttributeOutsideInit
class Alias:			# OrderedDict members
	defaultFont = None
	disabledFont = None
	...
	def __repr__(self):
		try:
			rep = f'Alias({self.name!r})'
			if -1 < self.location:
				rep += f'  (read at {self.location} in {con.CFGFILE})'
			if self.defn is None or len(self.defn):
				rep += f'\n    defn: {self.defn!r}'
			if len(self.value):
				rep += f'\n   value: {self.value!r}'
			if self.match:
				rep += f'\n    type {self.type!r}: {self.match!r}'
			if self.iifeArgs:
			...
			if self.button:
				...
				rep += f' button: {self.button._name}'
			rep += f', pollLead: {self.pollLead!r}'
			return rep
		except Exception as exc:
			print(exc)
			traceback.print_exc()
...
class AliasComment:
	...
	def __repr__(self):
		try:
			rep = f'\n   AliasComment({self.alias!r})'
			if self.tag is not None:
				rep += f', tag: {self.tag!r}'
			if self.offset is not None:
				rep += f' at offset: {self.offset!r}'
			if len(self.text):
				rep += f'\n    text ({self.length}): {self.text!r}'
			if self.tokenSpans:
				rep += f'\n    tokenSpans: {self.tokenSpans}'
			return rep
		except Exception as exc:
			msg = f'<<{exc}>>'
			print(exc)
...
def removeIIFEprop(alias):
	if connectedToOolite:
		cmd = f'delete console.script.{con.IIFE_PROPERTY_TAG + alias}'
		app.queueSilentCmd(cmd, f'del-{alias}-IProp')
