...
class _Found:
	...
	def formatLine(row, lines):
		fmt = f'{row + 1:>6}: {lines[row]}' # +1 as row is 0-indexed
...
def _appendFilesList(source, fileSize=None):
	...
	if source.inOxz:
		...
		if not reported:
			...
			oxz = f'in "{source.inOxz}":  {_fmtSize(oxzSize)}\n'
...
def _fmtFileTree(result):
	...
	path = path.replace('/', os.sep)
	if source.inOxz:
		rpt += f'found in: {source.inOxz}{2 * con.NL}'
		indent = 1
	else:
	...
	while len(folders):
		...
		rpt += f'{_indents[indent]}{folder}{os.sep}{con.NL}'
		indent += 1
...
def _fmtSize(fileSize):
	if fileSize > TERABYTE:
		return f'{fileSize/TERABYTE:,.2f} TB'
	elif fileSize > GIGABYTE:
		return f'{fileSize/GIGABYTE:,.2f} GB'
	elif fileSize > MEGABYTE:
		return f'{fileSize/MEGABYTE:,.2f} MB'
	elif fileSize > KILOBYTE:
		return f'{fileSize/KILOBYTE:,.2f} KB'
	return f'{fileSize} B'
