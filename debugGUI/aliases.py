...
class StackElement:
	...
	def _missing_method(self, repeat=None):
		raise ValueError(f'StackElement has no such method: {self.op!r}')
	...
	def __repr__(self):
		try:
			rpt = f'StackElement({self.op!r}, {self.alias!r}): '
			if self.op == 'nameClear':
				rpt += f'clear: {self.clear}, edited: {self.edited}'
			else:
	...
	if gv.aliasWindow.mouseXY:
		...
		try:
			...
			if aliasPosn is None:	# posn wrt appWindow
				...
				geom = f'{width}x{height}+{posX}+{posY}'
...
def newAliasAdd(event=None):			# <Return> handler in aliasNameEntry
	...
	if obj is None:
		...
		return 'break'
	if len(inText) == 0:
		gv.aliasMsgStr.set(f'define new alias "{alias}"')
		editAlias()
	elif obj.defn != inText:
		gv.aliasMsgStr.set(f'editing alias "{alias}"')
		editAlias()
	return 'break'
...
def _emphasizeLine(lineNum, offset, obj):
	...
	try:
		txt = gv.aliasDefn
		error = txt.index(f'{lineNum}.{offset}')
		start = txt.index(f'{lineNum}.0')
...
def _haveEditsPending():
	...
	if gv.aliasDefn.edit_modified() and not defnSaved:
		msg = f'The existing alias "{inEntry}" is not saved.'
...
def aliasUndo(undo=None): 				# handler for 'Undo' button
	...
	if undo.op == 'delete':
		...
		if obj:
			...
			if orphan:
				print(f'aliasUndo, found orphaned button {orphan!r}')
				pdb.set_trace()
				# orphan.destroy()
		...
		if undo.clear: # redo an undone clearText -> clearText
			...
			gv.aliasMsgStr.set(f'"{undo.alias}" restored')
		setAliasPollButton(undo.alias)
		_setAliasToMenuButton(undo.alias)
...
def _checkAliasName(alias=None):
	...
	while True:
		...
		if alias not in gv.aliases and alias in gv.scriptProps:
			gv.aliasMsgStr.set(f'{alias!r} already in use')
			gv.aliasNameVar.set('')
			break
		return alias
...
def _checkAliasText(alias, isNew):
	...
	if len(inText) == 0:
		if isNew:
			gv.aliasMsgStr.set(f'enter definition for {alias!r}')
			return None
		else:							# only when len==0 so don't clobber edits
			gv.aliasMsgStr.set(f"alias {alias!r} already exists")
...
def checkLoadedAliases():				# validate aliases loaded from CFGFILE
	...
	for alias, obj in list(gv.aliases.items()):
		...
		if alias is None:
			...
			msg = f'invalid alias {alias!r} in {con.CFGFILE}, discarding'
			gv.app.colorPrint(msg, emphasisRanges=[15, len(alias)])
			reason = gv.aliasMsgStr.get()
			gv.app.colorPrint(f'  reason: {reason}',
								emphasisRanges=[10,len(reason)])
			msgCount += 1
		defn = obj.defn
		if len(defn) == 0:
			if msgCount > 0: gv.app.colorPrint('')
			msg = f'missing definition for {alias!r} in {con.CFGFILE}'
		...
		if fixed == False:
			...
			if len(nls) > 20:
				gv.app.colorPrint(f'  {fixed[:nls[10]]}')
				gv.app.colorPrint(' ... (report abridged as > 20 lines)')
				gv.app.colorPrint(f'  {fixed[nls[-10]:]}')
			else:
				gv.app.colorPrint(f'  {fixed}')
			msgCount += 1
			if alias:
				obj.defn = fixed
...
def _editsPending(widget=None):			# display warning if edits will be lost
	...
	msg = f'Your edits to {inEntry!r} have not been saved.\n'
...
def _aliasUndoDelete(undo):		# insert previously deleted/edited alias
	...
	try:
		...
		obj.inMenu = undo.button is not None
		gv.aliasNameVar.set(alias)
		gv.aliasMsgStr.set(f'"{alias}" restored')
...
def _aliasDBremove(alias):				# remove alias from data base
	...
	if _fetchAliasName() == alias:# user still has it selected
		clearAliasEntry()
		_clearAliasTextBox()
	gv.aliasMsgStr.set(f'"{alias}" deleted')
...
def _mkIIFEObjectCmd(alias, defn):						# mk silent cmd for an iife
	...
	if gv.formattingAliases:
		cmd += f'? console.script.{iifeProp}.toString() : null; '
...
def _mkFnObjectCmd(alias, defn):		# mk silent commands for a function
	...
	cmd = f'Object.defineProperty( console.script, "{alias}", '
	cmd += f'{{ value: {defn}, '
	cmd += 'enumerable: true, configurable: true, writable: true } ) '
	if gv.formattingAliases:
		cmd += f'? console.script.{alias}.toString(): null; '
	# else:							# don't return function's code
	# 	cmd += '? true : null; '
	return cmd
...
def _sendAliasRegistration(alias, poll=False):
	...
	if alias in gv.aliases:
		...
		if obj.match and obj.type == 'func':
			...
			cmd = f'Object.defineProperty(console.script, "{alias}", '
			cmd += '{ enumerable: true, configurable: true, writable: true, '
			cmd += f'  value: typeof( {defn} ) == "function" ? '
	...
	if alias in gv.aliases:
		if gv.connectedToOolite:
			...
			if alias in _execQueue:
				_execQueue.remove(alias)
			cmd = f'delete console.script["{alias}"]'
			gv.app.queueSilentCmd(cmd, f'alias-{alias}-remove')
...
def _reportAll(message, caller):
	print(f'_reportAll, {caller}, lastCommand:')
	lines = gv.lastCommand.split(con.NL)
	for line in lines:
		print(line)
	print(f'_reportAll, {caller}, message:')
...
def rptAliasError(msgTag, message, appPrint=None):
	...
	if op == 'poll':
		...
		if gv.sessionInitialized == 0 and not _pollErrorStarted:
			...
			msg += f'Oolite reports error(s) in {con.CFGFILE} \n'
		msg += f'for alias {alias!r}, \t{errmsg}'
