...
def monitorResolutions():
	...
	gv.geometry = gv.root.winfo_geometry()
	gv.root.attributes('-fullscreen', False)
	gv.root.state('normal')
	gv.root.update_idletasks()
	gv.monitorsWidth, gv.monitorsHeight = mu.getAppDimns(gv.geometry)
...
def setAppTitle(tcp):
	gv.root.title(f'{con.DEBUGGER_TITLE} ({tcp})')
