...
class TopWindow(tk.Toplevel):
	...
	def center(self):
		...
		self.geometry(f'{winWidth}x{winDepth}+{winXoff}+{winYoff}')
...
class ToolTip(tk.Toplevel):
	...
	def _leaveTip(self, event=None): # toolTip's <Leave> handler
		...
		event.widget if event and hasattr(event, 'widget') else None
	...
	def _showTip(self):
		...
		self._tip.geometry(f'+{destX}+{destY}')
...
class OoCombobox(ttk.Entry):
	...
	def placePopdown(self, reqH):
		...
		if y + h + reqH > self.popdown.winfo_screenheight():
			...
			geomY = y + h
		self.toplevel.wm_geometry(f'{w}x{reqH}+{x}+{geomY}')
...
class OoSelector(OoCombobox):
	...
	def removeFromList(self, event=None, value=None, clear=True):
		super().removeFromList(event, value)
		if self.selectors > 0:
			self.checkSelectionCount()

	def addEntry(self, value):
		super().addEntry(value)
		if self.selectors > 0:
			self.checkSelectionCount()
...
class SearchBox(TopWindow):
	...
	def startSearch(cls, counting=False, marking=False):
		...
		if user.lastSearchIdx == '':	# first time visit
			...
			searchFrom = user.lastSearchIdx if backwards \
										else f'{user.lastSearchIdx} +1c'
		idx = searchFrom if backwards else f'{searchFrom} -1c'
		...
		while not found or counting or marking:
			searchFrom = idx if backwards else f'{idx} +1c'
			idx = ''
			try:
			...
			endIdx = txt.index(f'{idx}+{foundLength}c')
			if wordsOnly:
				if txt.compare(idx, '!=', f'{idx} wordstart') \
						or txt.compare(f'{endIdx} +1c', '!=',
										f'{endIdx} wordend'):
	...
	def openSearchBox(cls, centered=False):		# command for popup 'Search ...'
		...
		if len(user.searchPrefix):
			searchTitle = f'{user.searchPrefix} search'
...
class ScrollingText(tk.Text):
	...
	def needsScrollbars(self):
		...
		needsVertical = self.compare(start, '>', '1.0') \
						or self.compare(self.index(f'@0,{height}'),
										'<', lastLine)
		...
		stop = self.index(f'@{width}, {height} lineend')		# the last visible line
		...
		if hScrollMapped:
			...
			stop = self.index(stop + f' + {covered} lines lineend')
			if self._dbg:
		...
		if self._dbg:
			print(f'needsHscrollbar, widthExceeded({start}, {covered}, -1) is {chkTop}')
		if chkTop:
			return needsVertical, True
		chkBottom = widthExceeded(stop, covered + 1, 1)
		if self._dbg:
			print(f'needsHscrollbar, widthExceeded({stop}, {covered}, 1) is {chkBottom}')
		if chkBottom:
			return needsVertical, True
		lineIdx = start
		while True:
