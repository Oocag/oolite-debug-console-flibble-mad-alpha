...
def nextVersion(fn, ext, highest=None):
	...
	try:
		...
		if currVer or vern + 1 == maxVerNum:
			errmsg += f'deleting {currVer!r} '
		else:
			errmsg += f'renaming {nextFn!r} '
...
def initConfig():						# initialize config dict with defaults
	...
	try:
		...
		if exc.errno == errno.ENOENT:
			...
			errmsg = f'IOError loading configuration: {exc!r}'
			gv.debugLogger.exception(errmsg)
...
def _readCfgFile():
	...
	errmsg = ''
	try:
		with open(con.CFGFILE, encoding='utf-8') as fp:
			lines = fp.read()
		...
		while su.inbounds(cfgIdx, config):
			...
			if section:
				...
				if section['name'] != 'Aliases':
					...
					gv.formattingAliases = gv.CurrentOptions['Settings'].get('FormatAliasFns', False)
	...
	try:
		def _setCfg(cfgVal):
			...
			if isinstance(defaultVal, bool):
				...
				if isinstance(cfgVal, (list, tuple)):
					cfgVal = '[{}]'.format(', '.join([str(cv) for cv in cfgVal]))
				else:
					cfgVal = cfgVal
