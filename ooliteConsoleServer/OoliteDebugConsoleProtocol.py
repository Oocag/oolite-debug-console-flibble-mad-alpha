...
class OoliteDebugConsoleProtocol (PropertyListPacketProtocol):
	...
	def badPListSend(self, plist):
		consoleLogger.warning(f"Attempt to send bad plist: {plist}")
				...
				for k in packet[P.removedConfigurationKeysKey]:
					try:
						...
						if isinstance(exc, KeyError):
							print(f'key {k!r} not found in __configuration',
								  file=sys.stderr)
	...
	def __unknownPacket(self, _type, packet):
		...
		consoleLogger.error(f'Unkown packet type "{_type}", ignoring.')
